...
import type { TemplateRow, TemplateValues } from "@/lib/actions/templates";
...
export function TemplatesPage({ initialTemplates }: Props) {
  ...
  if (templateSnapshot.source !== initialTemplates) {
    setTemplateSnapshot({ source: initialTemplates, templates: initialTemplates });
  }

  const templates = templateSnapshot.templates;
  const setTemplates = (update: Template[] | ((currentTemplates: Template[]) => Template[])) => {
    setTemplateSnapshot((current) => ({
      source: current.source,
      templates: typeof update === "function" ? update(current.templates) : update,
    }));
  };
  ...
  async function handleDelete(id: string) {
    ...
    if (!result.ok) { setTemplates(previous); toast.error(result.error); return; }
  ...
  function TemplateCard({ t }: { t: Template }) {
    return (
      <div className="bg-white border border-[#E5E7EB] rounded-xl p-4 space-y-2 hover:shadow-sm transition-shadow">
        <div className="flex items-start justify-between gap-2">
          <div>
            <p className="font-semibold text-[#0B1220] text-sm">{t.name}</p>
            {t.description && <p className="text-xs text-[#64748B] mt-0.5">{t.description}</p>}
          </div>
          <div className="flex gap-1 shrink-0">
            <button onClick={() => handleDuplicate(t)} className="h-7 w-7 flex items-center justify-center rounded hover:bg-[#F1F5F9]" title="Dupliquer"><Copy className="h-3.5 w-3.5 text-[#64748B]" /></button>
            <button onClick={() => openEdit(t)} className="h-7 w-7 flex items-center justify-center rounded hover:bg-[#F1F5F9]"><Edit className="h-3.5 w-3.5 text-[#64748B]" /></button>
        ...
        {t.type === "devis" && (
          <p className="text-xs text-[#64748B]">{(t.content.items as unknown[])?.length ?? 0} lignes</p>
        )}
        {t.type === "contract_clause" && (
          <p className="text-xs text-[#64748B]">{(t.content.clauses as unknown[])?.length ?? 0} clause{(t.content.clauses as unknown[])?.length > 1 ? "s" : ""}</p>
        )}
        {t.type === "checklist" && (
          <p className="text-xs text-[#64748B]">{(t.content.items as unknown[])?.length ?? 0} éléments</p>
        )}
    ...
    <div className="space-y-4">
      <Tabs defaultValue="devis">
        <TabsList className="h-9 bg-[#F1F5F9] border border-[#E5E7EB] p-0.5 rounded-lg">
          <TabsTrigger value="devis" className="gap-1.5 text-[13px] rounded-md data-[state=active]:bg-white data-[state=active]:shadow-none data-[state=active]:text-[#0B1220] text-[#64748B]">
            <FileText className="h-3.5 w-3.5" />Devis ({devisTemplates.length})
          </TabsTrigger>
          <TabsTrigger value="clauses" className="gap-1.5 text-[13px] rounded-md data-[state=active]:bg-white data-[state=active]:shadow-none data-[state=active]:text-[#0B1220] text-[#64748B]">
            <AlignLeft className="h-3.5 w-3.5" />Clauses ({contractTemplates.length})
          </TabsTrigger>
          <TabsTrigger value="checklists" className="gap-1.5 text-[13px] rounded-md data-[state=active]:bg-white data-[state=active]:shadow-none data-[state=active]:text-[#0B1220] text-[#64748B]">
              ...
              <div className="space-y-2">
                ...
                {checklistItems.map((item, i) => (
                  <div key={i} className="flex gap-1 items-center">
                    <span className="text-xs text-[#64748B] w-5 text-right shrink-0">{i + 1}.</span>
