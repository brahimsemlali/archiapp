...
const STATUS_STYLE: Record<string, string> = {
  a_faire:  "bg-[#F1F5F9] text-[#64748B] border-[#E5E7EB]",
  ...
  return (
    <Sheet open={!!initialTask} onOpenChange={(open) => !open && onClose()}>
      <SheetContent className="w-full sm:max-w-lg overflow-y-auto p-0">
        {/* Header */}
        <div className="sticky top-0 z-10 bg-white border-b border-[#E5E7EB] px-5 py-3 flex items-center justify-between">
          <button
            onClick={onClose}
            className="flex items-center gap-1.5 text-sm text-[#64748B] hover:text-[#0B1220] transition-colors"
          >
            <X className="h-4 w-4" />
            Fermer
          </button>
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              disabled={!!deletingId}
              className="p-1.5 rounded-lg hover:bg-[#FCEFE6] text-[#64748B] hover:text-[#C75B2E] transition-colors"
              title={t("delete")}
            >
        ...
        <div className="px-5 py-5 space-y-6">
          ...
          <SheetHeader className="p-0">
            <SheetTitle className="text-[18px] font-semibold text-[#0B1220] leading-snug tracking-tight text-left">
              {initialTask.title}
            </SheetTitle>
          </SheetHeader>
          ...
          <button
            ...
            className={cn(
              ...
              initialTask.status === "termine"
                ...
                : "bg-[#0B1220] text-[#F7F8FA] hover:bg-[#2C2D24]"
            )}
          >
            ...
            <div className="space-y-2.5">
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-1.5 w-28 shrink-0">
                  ...
                  <span className="text-[13px] text-[#64748B]">Statut</span>
                </div>
                <span className={cn(
                  "text-[11.5px] font-medium px-2.5 py-1 rounded-full border",
                  STATUS_STYLE[initialTask.status] ?? "bg-[#F1F5F9] text-[#64748B] border-[#E5E7EB]"
                )}>
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-1.5 w-28 shrink-0">
                  ...
                  <span className="text-[13px] text-[#64748B]">Priorité</span>
                </div>
                <span className={cn(
                  "text-[11.5px] font-medium px-2.5 py-1 rounded-full border",
              ...
              {initialTask.due_date && (
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1.5 w-28 shrink-0">
                    ...
                    <span className="text-[13px] text-[#64748B]">Échéance</span>
                  </div>
                  <span className={cn(
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-1.5 w-28 shrink-0">
                  ...
                  <span className="text-[13px] text-[#64748B]">Estimé</span>
                </div>
                {editingEstimate ? (
                  <form
                  ...
                  >
                    <input
                      ...
                      className="text-[13px] border border-[#E5E7EB] rounded-lg px-2.5 py-1 w-28 focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white"
                    />
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      meta.estimated_minutes
                        ? "text-[#0B1220] border-[#E5E7EB] hover:border-blue-300"
              ...
              {initialTask.projects?.title && (
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1.5 w-28 shrink-0">
                    ...
                    <span className="text-[13px] text-[#64748B]">Projet</span>
                  </div>
                  <span className="text-[13px] text-[#0B1220]">{initialTask.projects.title}</span>
                </div>
              )}
              ...
              {initialTask.clients?.name && (
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1.5 w-28 shrink-0">
                    ...
                    <span className="text-[13px] text-[#64748B]">Client</span>
                  </div>
                  <span className="text-[13px] text-[#0B1220]">{initialTask.clients.name}</span>
                </div>
              )}
            </div>
          </div>
          ...
          {initialTask.description && (
            <div>
              ...
              <p className="text-[13.5px] text-[#1E293B] leading-relaxed whitespace-pre-wrap">
                {initialTask.description}
              </p>
            </div>
          )}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-[#ADAB9D]">
                Checklist
                {totalCount > 0 && (
                  <span className="ml-2 normal-case text-[#64748B]">{doneCount}/{totalCount}</span>
                )}
              </p>
              {savingMeta && (
            ...
            {totalCount > 0 && (
              <div className="mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] text-[#64748B] tabnum w-7 text-right">{progressPct}%</span>
                </div>
              </div>
            )}
            ...
            <div className="space-y-1">
              {checklist.map((item) => (
                <div key={item.id} className="flex items-center gap-2.5 group py-1 px-1 rounded-lg hover:bg-[#F7F8FA]">
                  <button
                  ...
                  <span className={cn(
                    ...
                    item.done ? "line-through text-[#ADAB9D]" : "text-[#1E293B]"
                  )}>
                    {item.text}
                  </span>
                  <button
            ...
            {addingItem ? (
              <form onSubmit={handleAddItem} className="flex items-center gap-2 mt-2">
                <div className="shrink-0 w-4 h-4 rounded border border-[#E5E7EB]" />
                <input
                  ref={newItemRef}
                  value={newItemText}
                  ...
                  className="flex-1 text-[13px] border-0 border-b border-[#E5E7EB] focus:outline-none focus:border-blue-400 bg-transparent pb-0.5"
                />
                <button
                  type="submit"
              ...
              <button
                ...
                className="flex items-center gap-2 mt-2 text-[13px] text-[#64748B] hover:text-[#0B1220] transition-colors group"
              >
          ...
          <div>
            ...
            {blockedByTasks.length > 0 && (
              <div className="space-y-1 mb-2">
                {blockedByTasks.map((dep) => (
                  <div key={dep.id} className="flex items-center gap-2 py-1 px-2 bg-[#F7F8FA] rounded-lg group">
                    <Link2 className="h-3 w-3 text-[#64748B] shrink-0" />
                    <span className={cn(
                      "flex-1 text-[12.5px]",
                      dep.status === "termine" ? "line-through text-[#ADAB9D]" : "text-[#1E293B]"
                    )}>
                      {dep.title}
                    </span>
                    <span className={cn(
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 text-[13px] text-[#64748B] hover:text-[#0B1220] transition-colors"
              >
                <Plus className="h-3.5 w-3.5" />
              ...
              {showDepPicker && (
                <div className="absolute left-0 top-full mt-1.5 w-72 bg-white border border-[#E5E7EB] rounded-xl shadow-lg z-20 overflow-hidden">
                  <div className="p-2 border-b border-[#E5E7EB]">
                    <input
                      ...
                      className="w-full text-[13px] px-2 py-1.5 rounded-lg bg-[#F7F8FA] focus:outline-none focus:ring-1 focus:ring-blue-400"
                    />
                  </div>
                    ...
                    {depCandidates.length === 0 ? (
                      ...
                      depCandidates.slice(0, 20).map((t) => (
                        <button
                          ...
                          className="w-full flex items-center gap-2.5 px-3 py-2.5 hover:bg-[#F7F8FA] text-left transition-colors"
                        >
                          <span className={cn(
                          ...
                          <span className="flex-1 text-[13px] text-[#0B1220] truncate">{t.title}</span>
                          {t.projects?.title && (
