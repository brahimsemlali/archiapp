...
- **File uploads = direct-to-storage signed URLs, NEVER server-action FormData bytes.** Vercel caps server-action request bodies at ~4.5 MB, so passing file bytes through a server action silently fails for real plans/renders/photos/docs. Pattern: a `create…UploadUrlAction(meta {name,type,size})` validates + `createSignedUploadUrl`; the browser `uploadToSignedUrl`s the bytes (`@/lib/supabase/client`); a `finalize…Action` records the row (re-validate ownership + path prefix; for public/portal uploads also verify the object exists). Live in files/visites/moodboards/projects(inspirations)/portal. Only `logo.ts` (2 MB) still uses server-action upload — under the cap.
...
*Update 2026-06-16 — **W6 shipped: contract AI per jurisdiction (mechanism)** (worldwide.md W6). `getContractPrompt(localization)` in `src/lib/ai/prompts/contract.ts` picks grounding by country: Morocco returns the existing Loi 016-89 prompt + userPrompt **byte-identically** (flagship, unchanged, unit-guarded); every other country gets `internationalContractSystemPrompt({taxLabel,currency})` — jurisdiction-neutral, currency/tax-aware, that **actively forbids** citing any law/code/article/professional body (the real risk is a model citing real unvetted FR/UAE law, not inventing it) and forces `[À COMPLÉTER]` governing-law placeholders. The "review by local counsel" disclaimer is a contract **body section** (survives PDF/e-sign), not UI chrome. Stores version (MA "v1.1" unchanged) + `contracts.metadata.{ai_country,ai_jurisdiction,ai_prompt_version}` + country in usage log. Per-market grounding (DZ/TN/FR/Gulf) is **juriste-gated/deferred**; mechanism is live. 4 new tests (53 total). **W1–W6 = the worldwide engineering foundation; remaining (W7+) is juriste/market-gated, not code.** Next priority returns to the ship blockers (Supabase Pro, LemonSqueezy go-live, auth gate, legal finalization).*

*Update 2026-06-16 — **W5 shipped: documents & numbering per country** (worldwide.md W5). Document number prefixes come from the country pack (`invoicePrefix`/`quotePrefix`): francophone MA/DZ/TN/FR keep **FA/DEV** (the existing series — DEV not DV), AE/SA/INTL use INV/QUO; `nextFactureNumber`/`nextDevisNumber` resolve via `getWorkspaceLocalization`→`getCountryPack`. Legal sequentiality is untouched (the `next_workspace_document_number` RPC keys its counter on document_type; prefix is presentational). PDF firm-identity block is now pack-driven via `getFirmIdentityLines(firmRow)` + `CountryPack.firmIdentityFields` — **MA invoices/devis now print ICE/RC/IF/Patente** (was ICE-only; real compliance gap), wired through both templates + all 5 PDF paths incl. the factures sent-snapshot (snapshot select now captures `country`). Identity labels are jurisdiction terms, NOT locale-translated. Packs without backing columns render no identity lines (no invented SIRET/TRN — deferred). Legal mentions deferred (juriste). 5 new tests (49 total). Next: W6 (contract AI per jurisdiction — needs per-market legal grounding).*
